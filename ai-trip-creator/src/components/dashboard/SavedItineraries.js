...
const addSectionHeading = (doc, text, yPosition, color = "#004ba8") => {
  doc.setFontSize(16)
  doc.setTextColor(color)
  doc.text(text, 20, yPosition)
}

// Helper to add content blocks
const addContentBlock = (doc, label, content, yPosition) => {
  doc.setFontSize(12)
  doc.setTextColor(0, 0, 0)
  doc.text(`${label}: ${content}`, 20, yPosition)
}
...
const generatePDF = (itinerary) => {
  ...
  // Add Title
  doc.setFontSize(20)
  doc.setTextColor("#004ba8")
  doc.text(itinerary.itineraryName, 105, 20, { align: "center" })
  ...
  y += 10
  addContentBlock(doc, "Number of Days", itinerary.numDays, y)
  y += 20
  ...
  itinerary.days.forEach((day, dayIndex) => {
    ...
    if (day.flights && day.flights.length > 0) {
      day.flights.forEach((flight, flightIndex) => {
        addContentBlock(doc, `Flight ${flightIndex + 1}`, `${flight.flightNumber} - Departs: ${flight.departure}, Arrives: ${flight.arrival}`, y)
        y += 10
      })
    }
    ...
    if (day.accommodation && day.accommodation.length > 0) {
      day.accommodation.forEach((acc, accIndex) => {
        addContentBlock(doc, `Accommodation ${accIndex + 1}`, `${acc.name} - Check-in: ${acc.checkin || "N/A"}, Checkout: ${acc.checkout || "N/A"}`, y)
        y += 10
      })
    }
    ...
    if (day.activities && day.activities.length > 0) {
      day.activities.forEach((activity, actIndex) => {
        addContentBlock(doc, `Activity ${actIndex + 1}`, `${activity.name} at ${activity.time}`, y)
        y += 10
      })
    }

    y += 20 // Add some space after each day
