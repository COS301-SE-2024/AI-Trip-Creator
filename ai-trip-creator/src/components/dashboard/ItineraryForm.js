...
//   const [errorMessage, setErrorMessage] = useState("");
//   const [userId, setUserId] = useState("");
//   const [maxOffers, setMaxOffers] = useState(9);
...
//       const departureFlights = await getFlightOffers(startLocation, endLocation, departureDate, 1, maxOffers);
//       setFlights(departureFlights);

//       if (returnFlight && returnDate) {
//         const returnFlightResults = await getFlightOffers(endLocation, startLocation, returnDate, 1, maxOffers);
//         setReturnFlights(returnFlightResults);
//       }
//     }
//   };
...
//   return (
//     <Box alignItems="center" marginLeft="280px">
//       <Box margin="20px">
//         <Typography variant="h4">Itinerary Form</Typography>
//         <Typography variant="h6" sx={{ marginTop: "20px" }}>Step 1: Flights</Typography>
...
//         <Button variant="outlined" onClick={() => setShowFlightSearch(true)} fullWidth sx={{ marginTop: "20px" }}>+ Add a Flight</Button>
...
//             <Button onClick={handleSearchFlights} variant="contained" color="primary" sx={{ marginTop: "20px" }}>Search Flights</Button>
//             {errorMessage && <Alert severity="error" sx={{ marginTop: "20px" }}>{errorMessage}</Alert>}

//             <Typography variant="h6" sx={{ marginTop: "20px" }}>Departure Flights</Typography>
//             <Grid container spacing={2} sx={{ marginTop: "10px" }}>
//               {flights.length > 0 ? (
//                 flights.map((flight, index) => (
//                   <Grid item xs={12} sm={6} md={4} key={index}>
//                     <Card onClick={() => handleFlightToggle(flight)} sx={{ cursor: 'pointer' }}>
//                       <CardContent>
//                         <Typography><FaPlaneDeparture /> {flight.itineraries[0].segments[0].departure.iataCode} to {flight.itineraries[0].segments[0].arrival.iataCode}</Typography>
//                         <Typography>{flight.price.total} {flight.price.currency} ({flight.priceInZar} ZAR)</Typography>
...
//             {flights.length >= 9 && (
//               <Button onClick={loadMoreFlights} variant="outlined" fullWidth sx={{ marginTop: "20px" }}>Load More Flights</Button>
//             )}

//             {returnFlight && (
//               <>
//                 <Typography variant="h6" sx={{ marginTop: "40px" }}>Return Flights</Typography>
//                 <Grid container spacing={2} sx={{ marginTop: "10px" }}>
...
//                         <Card onClick={() => handleFlightToggle(flight)} sx={{ cursor: 'pointer' }}>
//                           <CardContent>
//                             <Typography><FaPlaneArrival /> {flight.itineraries[0].segments[0].departure.iataCode} to {flight.itineraries[0].segments[0].arrival.iataCode}</Typography>
//                             <Typography>{flight.price.total} {flight.price.currency} ({flight.priceInZar} ZAR)</Typography>
//                             <Button variant="contained" sx={{ marginTop: "10px" }}>
//                               {selectedFlights.some((f) => f.id === flight.id) ? "Remove from Itinerary" : "Add to Itinerary"}
//                             </Button>
...
//         <Typography variant="h6" sx={{ marginTop: "40px" }}>Selected Flights</Typography>
//         <Grid container spacing={2} sx={{ marginTop: "10px" }}>
...
//                     <Typography><FaPlaneDeparture /> {flight.itineraries[0].segments[0].departure.iataCode} to {flight.itineraries[0].segments[0].arrival.iataCode}</Typography>
//                     <Typography>{flight.price.total} {flight.price.currency} ({flight.priceInZar} ZAR)</Typography>
//                     <Button variant="contained" onClick={() => handleFlightToggle(flight)} sx={{ marginTop: "10px" }}>
